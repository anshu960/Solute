...
import { getBusinessId, getUserId } from '../../services/authService';
  ...
  const CustomerDetails = () => {
    ...
    useEffect(()=>{
        ...
        if(sessionStorage.getItem(id) && sessionStorage.getItem("selectedCustomer")){
            setSales(JSON.parse(sessionStorage.getItem(id)));
            setSelectedCustomer(JSON.parse(sessionStorage.getItem("selectedCustomer")))
            setCustomerList([JSON.parse(sessionStorage.getItem("selectedCustomer"))])
    ...
    const refreshCustomerList=()=>{
      ...
      const refreshSalesList=(customer)=>{
        ...
        SendEvent(SocketEvent.RETRIVE_SALE,request,handleRetriveSalesEvent)
      }
      ...
      <Page title="Sale">
          <Fragment>
            ...
            <Box py={3} className={classes.bottomButtonExpert}>
                <ExportPDF refsToPrint={[tableRef]} totalAmount={totalAmount} />
            </Box>
            <Box py={3} className={classes.bottomButtonExpert}>
                <ExportPDF refsToPrint={[tableRef]} totalAmount={totalAmount} />
            </Box>
